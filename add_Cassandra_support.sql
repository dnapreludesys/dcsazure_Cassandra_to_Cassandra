...
/*
Issue:
 The previous encoding method using CONVERT(VARBINARY, ...) was collation-dependent and performed partial/truncated conversion 
 for long strings. Because of this, two different long column names:
customer.profile.identity.personal_info.government.ids.passport_number
customer.profile.identity.personal_info.government.ids.ssn
ended up producing the same hex value, since SQL Server only considered part of the string during conversion under the 
default collation. This caused collisions in the masking pipeline.


Fix:

 Replaced the encoding logic in generate_masking_parameters stored procedure with a binary-safe 
 UTF-8 conversion using COLLATE Latin1_General_100_BIN2_UTF8 and CAST(... AS VARBINARY(MAX)), 
 ensuring every column name produces a unique, deterministic, and environment-independent hex value. 
 (did not make changes to the existing one created a new SP with this modification - generate_cosmos_no_sql_masking_parameters)

*/

CREATE   PROCEDURE [generate_cassandra_masking_parameters]
...
BEGIN
    SET NOCOUNT ON;
    DECLARE @StoredProcedureVersion VARCHAR(13) = 'V2025.12.09.0';
    ...
    WITH base_ruleset_filter AS (
        SELECT
            ...
            CONCAT(
                ...
                CONVERT(
                    VARCHAR(MAX),
                    CAST(r.identified_column COLLATE Latin1_General_100_BIN2_UTF8 AS VARBINARY(MAX)),
                    2
                )
